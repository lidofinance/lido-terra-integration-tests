...
async function main() {
    ...
    //block 70 - 74
    // deregister oracle vote and waste 5 blocks
    const prevotesToClear2 = initialPrevotes[0]
    const votesToClear2 = initialVotes[0]
    ...
    fs.writeFileSync("6_block149_state.json", JSON.stringify(await mantleState.getState(), null, 2))
...
main()
    ...
    .then(async () => {
        console.log("saving state...")
        fs.writeFileSync("6_actions.json", JSON.stringify(getRecord(), null, 2))
        fs.writeFileSync("6_state.json", JSON.stringify(await mantleState.getState(), null, 2))
    })
    .catch(console.log)