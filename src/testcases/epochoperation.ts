...
async function main() {
    ...
    fs.writeFileSync("ee_block149_state.json", JSON.stringify(await mantleState.getState(), null, 2))


    await mustPass(moneyMarket.repay_stable(a, 1))
    ...
    //block 152
    //await mustPass(moneyMarket.borrow_stable(a, 200000000000, undefined))
    ...
    //block 168 - 200
    await mustPass(emptyBlockWithFixedGas(lcd, gasStation, 33))
...
main()
    ...
    .then(async () => {
        console.log("saving state...")
        fs.writeFileSync("ee_actions.json", JSON.stringify(getRecord(), null, 2))
        fs.writeFileSync("ee_state.json", JSON.stringify(await mantleState.getState(), null, 2))
    })
    .catch(console.log)