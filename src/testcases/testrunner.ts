...
export default async function isLCDReachable(port: number, host: string) {
  try {
    //we have to wait for some wallet from genesis
    return await axios
      .get(
        `http://${host}:${port}/cosmos/auth/v1beta1/accounts/terra19uv79nyv4a62slv3npkze3p9984ueafemaa352`
      )
      .then((res) => {
...
const isOracleReachable = async (port: number, host: string) => {
  try {
    return await axios
      .get(`http://${host}:${port}/oracle/denoms/exchange_rates`)
      .then((resp) => {
...
const configure_shared_testnet = async (
  walletPoolSize = 10
  ...
  const masterWallet = lcd.wallet(
    new MnemonicKey({ mnemonic: predefinedKeys[0] })
  );
  let msgs: Msg[] = [];
  for (let i = 0; i < walletPoolSize; i++) {
    ...
    msgs.push(
      new MsgSend(
        masterWallet.key.accAddress,
        wallet.key.accAddress,
        "100000000000000uusd,100000000000000uluna"
      )
    );
  }
...
const shared_concurrent_runner = async (
  tests: Array<(contracts?: Record<string, number>) => Promise<void>>,
  contracts?: Record<string, number>
) => {
  return Promise.all(tests.map((t) => t(contracts)));
};

const localtestnet_shared_testcases: Array<
  (contracts?: Record<string, number>) => Promise<void>
> = [
  BlunaShortTest,
  STlunaShortTest,
  ConversionTest,
