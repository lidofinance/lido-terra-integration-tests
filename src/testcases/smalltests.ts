...
async function main() {
        ...
        new MsgExecuteContract(
          ...
          anchor.moneyMarket.contractInfo["moneymarket_oracle"].contractAddress,
          {
            feed_price: {
              prices: [
                [
                  anchor.bAsset.contractInfo["anchor_basset_token"]
                    .contractAddress,
                  "1.000000",
                ],
              ],
  ...
  await anchor.bAsset.register_validator(
    ownerWallet,
  ...
  console.log({
    ...
    mmInterest:
      moneyMarket.contractInfo["moneymarket_interest"].contractAddress,
  ...
  //block 31
  await mustPass(
    basset.bond(a, 20000000000000, validators[0].validator_address)
  );
  ...
  await mustPass(basset.update_global_index);
  // block 57
  await basset.send_cw20_token(
    a,
    10000000000000,
  ...
  // await mustPass(emptyBlockWithFixedGas(lcd, gasStation, 45))
  ...
  // //block 121
  // await mustPass(moneyMarket.deposit_stable(b, 999999999999000000))
  ...
  // fs.writeFileSync("small_block149_state.json", JSON.stringify(await mantleState.getState(), null, 2))
