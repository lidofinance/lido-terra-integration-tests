...
export const getCoreState = async (
    ...
    const block = await makeQuery(gql`
        query {
            BlockState {
                Block {
                    Header {
    ...
    `, {}, client).then(r => ({
        ...
        block_proposer: b32.encode('terravaloper', b32.toWords(r.BlockState.Block.Header.ProposerAddress))
    }))

    const oracle_price = await makeQuery(gql`
        query {
    ...
    `, {}, client).then(r => r.Result)

    const reward = await makeQuery(gql`
    ...
    const validator_info: any = await (async () => {
        ...
        const fin = validator_list.StakingValidators.Result.map(async({ OperatorAddress, Commission }) => {
            const validator_specific = await makeQuery(gql`
            ...
            `, { OperatorAddress }, client)

            return {
                valaddr: OperatorAddress,
                commission: Commission.CommissionRates.Rate,
                rewardsPool: {
                    denoms: validator_specific.DistributionValidatorsValidatorAddrOutstandingRewards.Result.map(k => ({
                        denom: k.Denom,
                        amount: k.Amount,
                    }))
                },
                delegators: validator_specific.StakingValidatorsValidatorAddrDelegations.Result.reduce((m, k) => {
