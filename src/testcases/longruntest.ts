...
async function main() {
    ...
    await mustPass(testState.moneyMarket.borrow_stable(testState.wallets.b, 200000000000, undefined))
    await mustPass(testState.moneyMarket.borrow_stable(testState.wallets.c, 400000000000, undefined))

    //block 170
    await mustPass(testState.basset.update_global_index(testState.wallets.a))


    fs.writeFileSync("longruntest_state.json", JSON.stringify(await mantleState.getState(), null, 2));

    //block 171
    await mustPass(testState.moneyMarket.market_claim_rewards(testState.wallets.a))
    await mustPass(testState.moneyMarket.market_claim_rewards(testState.wallets.b))
    await mustPass(testState.moneyMarket.market_claim_rewards(testState.wallets.c))


    await mustPass(emptyBlockWithFixedGas(testState.lcdClient, testState.gasStation, 20000))
}
...
main()
    ...
    .then(async () => {
        ...
        fs.writeFileSync(
            "longruntest_action.json",
            JSON.stringify(getRecord(), null, 2)
        );
        fs.writeFileSync(
            "longruntest_state.json",
