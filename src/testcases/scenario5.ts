...
async function main() {
    ...
    //block 70 - 74
    // deregister oracle vote and waste 5 blocks
    const prevotesToClear2 = previousVote
    const votesToClear2 = previousPrevote
    ...
    console.log("saving state...")
    fs.writeFileSync("5_block149_state.json", JSON.stringify(await mantleState.getState(), null, 2))
...
main()
    ...
    .then(async () => {
        console.log("saving state...")
        fs.writeFileSync("5_actions.json", JSON.stringify(getRecord(), null, 2))
        fs.writeFileSync("5_state.json", JSON.stringify(await mantleState.getState(), null, 2))
    })
    .catch(console.log)