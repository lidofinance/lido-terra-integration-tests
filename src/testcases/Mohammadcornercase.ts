...
async function main() {
    ...
    await mustPass(basset.bond(a, 1000, validators[0].validator_address))
    await mustPass(basset.bond(a, 1500, validators[1].validator_address))

    await mustPass(emptyBlockWithFixedGas(lcd, gasStation, 15))

    await mustPass(basset.send_cw20_token(a, 2000, { unbond: {} }, basset.contractInfo["anchor_basset_hub"].contractAddress))
}
...
main()
    ...
    .then(async () => {
        ...
        fs.writeFileSync("mohammad_actions.json", JSON.stringify(getRecord(), null, 2))
        fs.writeFileSync("mohammad_state.json", JSON.stringify(await mantleState.getState(), null, 2))
    })
    .catch(console.log)